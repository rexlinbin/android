package com.boxuu.gamebox.download;
...
public class DownloadManager {
        ...
        try {
            ...
            if (downloadInfoList != null && downloadInfoList.size() > 0) {
            	for (int i = 0; i < downloadInfoList.size(); i++) {
            		if (downloadInfoList.get(i).getState() == State.LOADING
            				|| downloadInfoList.get(i).getState() == State.WAITING) {
            			downloadInfoList.get(i).setState(State.CANCELLED);
            		}
            	}
			}
        } catch (DbException e) {
    ...
    public void addNewDownload(String url, String fileName, String target,
                               ...
                               final RequestCallBack<File> callback, String app_idString, String app_urlString) throws DbException {
    ...
    public class ManagerCallBack extends RequestCallBack<File> {
        ...
        public void onFailure(HttpException error, String msg) {
        	Toast.makeText(mContext, "下载失败，请检查网络设置或地址", Toast.LENGTH_SHORT).show();
            HttpHandler<File> handler = downloadInfo.getHandler();
            if (handler != null) {
                downloadInfo.setState(handler.getState());
            }
