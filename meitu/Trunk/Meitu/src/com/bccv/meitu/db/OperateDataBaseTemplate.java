...
public class OperateDataBaseTemplate {
	 ...
	 */
	public void open() {
		sSQLiteDatabase = MeituDataBase.getInstance().openDatabase(false);
		if (sSQLiteDatabase == null) {
	...
	public void open(boolean isIgnoreLocked) {
		sSQLiteDatabase = MeituDataBase.getInstance().openDatabase(
				isIgnoreLocked);
		if (sSQLiteDatabase == null) {
	...
	public int delete(Uri uri, String whereClause, String[] whereArgs) {
		try {
			...
			e.printStackTrace();
			Logger.e(TAG, "delete", "CommonDataBaseTemplate delete" + e.getMessage()
					+ "");
		} finally {
		}
		return 0;
	...
	public int update(Uri uri, ContentValues values, String whereClause,
		...
		try {
			...
			e.printStackTrace();
			Logger.e(TAG, "update", "CommonDataBaseTemplate update" + e.getMessage()
					+ "");
		} finally {
	...
	protected long insertWithOnConflict(String table, String nullColumnHack,
		...
		try {
			...
			Logger.w(TAG, "insertWithOnConflict",
					"CommonDataBaseTemplate insertBySql" + e.getMessage() + "");
		}
		return -1;
	}
	...
	protected Cursor select(String sql) {
		try {
			...
			e.printStackTrace();
			Logger.e(TAG, "select", "CommonDataBaseTemplate select" + e.getMessage()
					+ "");
		}
		return null;
	}
