...
//			Toast.makeText(this, "author_id wrong", 0).show();
	...
	private void initView(){
		...
		album_chat = (ImageView) findViewById(R.id.album_chat);
...
//		getPagerHight();
		...
		album_chat.setOnClickListener(this);
	...
	protected void setViewData(boolean refreshAlbum,boolean refreshCom) {
		...
		if(!refreshAlbum && !refreshCom){
			if (isLogin) {
				...
				if (sModule.isCared(String.valueOf(author_id))) {
					pro_guanzhu.setBackgroundResource(R.drawable.per_yiguanzhu);
				}else{
					pro_guanzhu.setBackgroundResource(R.drawable.per_guanzhu);
				}
...
//			Toast.makeText(getApplicationContext(), "获取数据失败", Toast.LENGTH_SHORT).show();
	...
	public void onClick(View v) {
		...
		case R.id.pro_back://返回
			onBackPressed();
			break;
		case R.id.album_chat:
			if (isLogin) {
				mHandler.post(new Runnable() {
	                @Override
	                public void run() {
	                	int userId = author_id;
	                    final String title = author_name;
	                    ...
	                    if (im == null) {
	                    	RCUtil.connect(UserInfoManager.getUserToken(), new RCConnectCallback() {
								...
								public void onSuccess(String userId) {
									if (RongIM.getInstance() != null) {
										RCUtil.setCommunicationInfo(String.valueOf(author_id), author_name, author_icon);
										RongIM.getInstance().startPrivateChat(ProZoneActivity.this, String.valueOf(userId), title);
									}
								}
								@Override
							...
							RCUtil.setCommunicationInfo(String.valueOf(author_id), author_name, author_icon);
							im.startPrivateChat(ProZoneActivity.this, String.valueOf(userId), title);
						}
	                }
				});
			}else{
...
//			imageView.startAnimation(animation);
			if (currIndex == 1) {
				...
				pro_line_left.setBackgroundColor(0xfff0f0f0);
			}else if(currIndex == 0){
				pro_line_right.setBackgroundColor(0xfff0f0f0);
