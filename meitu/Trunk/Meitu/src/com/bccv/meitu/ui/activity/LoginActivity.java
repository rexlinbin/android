...
//				mWaitting.setVisibility(View.VISIBLE);
		...
		NetWorkAPI.login(mContext, user.nickName, user.headPic,
			...
			public void onResult(NetResBean response) {
				...
				if(response.success && response instanceof LoginResBean){
					...
					Logger.i(TAG, "onResult", "userinfo : " + data);
					
					//TODO 登录融云
					RCUtil.connect(data.getToken(), new MyRCConnectCallback(data,user.snsType));
	...
	private class MyRCConnectCallback implements RCConnectCallback{
		
		private LoginResBean data;
		private int snsType;

		public MyRCConnectCallback(LoginResBean data,int snsType){
			this.data = data;
		...
		public void onSuccess(String userId) {
			saveUserInfo2Local(data,snsType);
			showShortToast("登录成功");
			finish();
			showWaiting(false);
		}
