		...
		case R.id.album_send_btn:
			if (UserInfoManager.isLogin()) {
				...
				if (trim == null || trim.equals("")) {
					...
					commentsList.add(trim);
					Comment com = new Comment();
					com.setComments_content(trim);
					com.setCtime((int)(System.currentTimeMillis()/1000));
					com.setId(UserInfoManager.getUserId());
					com.setUser_icon(UserInfoManager.getUserIcon());
					com.setUser_name(UserInfoManager.getUserName());
					tempList.add(0,com);
					commentList.add(0, com);
					album_et_comment.setText("");
					imm.hideSoftInputFromWindow(v.getWindowToken(), 0);
					mAdapter.setList(commentList,true);
//				showShortToast("收集评论");
				}
			}else{
				showShortToast("请先登录");
	...
	protected void getAllData(GetAuthorCommentResBean data,boolean isRefresh) {
		...
		if (tempList != null && tempList.size() > 0) {
			for (int i = 0; i < tempList.size(); i++) {
				commentList.add(0, tempList.get(i));
			}
		}
		setView(isRefresh);
	}
