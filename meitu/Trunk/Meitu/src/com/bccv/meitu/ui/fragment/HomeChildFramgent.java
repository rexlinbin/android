...
public class HomeChildFramgent extends Fragment implements IXListViewListener {
	...
	private boolean loginState = UserInfoManager.isLogin();
...
//	}
	...
	public void onResume() {
		...
		if(cate != 3 && isFirst && 
			...
			initData();
			isFirst = false;
		}else if(cate == 3 && (isFirst || UserInfoManager.isLogin()!= loginState)
					&& SystemUtil.isNetOkWithToast(ApplicationManager.getGlobalContext())){
				loginState=UserInfoManager.isLogin();
				isFirst = false;
				initData();
		}
	}
	...
	private synchronized void getNetData(int page,final boolean isRefresh){
		...
		if(SystemUtil.isNetOkWithToast(getActivity().getApplicationContext())){
			...
			if(cate!=3 || (cate==3 && loginState)){//如果不是关注 或者 是关注页面而且已登录
				Logger.v(TAG, "getNetData", "response.success : cate!=3 || (cate==3 && loginState) ");
				NetWorkAPI.getlist(getActivity().getApplicationContext(), cate, page, new HttpCallback() {
					...
					public void onResult(NetResBean response) {
						...
						if(response.success && response instanceof GetlistResBean){
...
//							}
			...
			}else if(cate==3 && !loginState){
				//TODO 获取本地数据
				ArrayList<String> localCareList = StorageModule.getInstance().getLocalCareList();
				Logger.v(TAG, "getNetData", " localCareList : " + localCareList);
				if(localCareList!=null&&localCareList.size()>0){
					NetWorkAPI.getLocalAttentionAlbumList(getActivity(), localCareList, new HttpCallback() {
	...
	public void onLoadMore() {
		...
		if(currentPage>=totalPage){
			Toast.makeText(getActivity().getApplicationContext(), "没有更多专辑了", Toast.LENGTH_SHORT).show();
			listView.stopLoadMore();
			listView.hideFooter();
			return;
		}
