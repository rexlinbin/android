...
public class SystemUtil {
	 ...
	 * @return
	 */
	public static String getMeiTuCacheFolder(int type) {
		String status = Environment.getExternalStorageState();
			...
			for (int i = 0; i < arrayPath.size(); i++) {
				...
				if (path.equals(sysInternalPath) && new File(path).exists()) {// 澶栫疆sd
					...
					if (!path.equals(sysInternalPath)
						...
						Logger.v(TAG, "saveToPathMap", "HS-U939 INTERNAL="
								+ path);
			...
			for (int i = 0; i < arrayPath.size(); i++) {
				...
				if (path.equals(sysInternalPath)) { // 外置sd
					if (new File(path).exists() && new File(path).canWrite()) {
						...
						Logger.v(TAG, "saveToPathMap",
								"not write INTERNAL_PATH=" + path);
					}
				} else {// 外置sd
					if (new File(path).exists() && new File(path).canWrite()
							&& filterStoragePath(path)) {
	...
	public static String getChannelCode() {
		String code = "own";
		try {
			ApplicationInfo info=ApplicationManager.getGlobalContext().getPackageManager()
					.getApplicationInfo(ApplicationManager.getGlobalContext().getPackageName(),PackageManager.GET_META_DATA);
			code =info.metaData.getString("channel");
		} catch (Exception e) {
			Logger.e(TAG, "getChannelCode", e.getClass().getSimpleName());
			code = "own";
		}
		return code;
	}
	...
	public static String getPhoneUdid(){
		...
		udidStr = ((TelephonyManager) ApplicationManager
				.getGlobalContext().getSystemService(Context.TELEPHONY_SERVICE))
				.getDeviceId();
