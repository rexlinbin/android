...
import com.bccv.meitu.sns.tencent.weixin.WeiXinShareUtil;
import com.bccv.meitu.sns.util.ApiClient;
import com.bccv.meitu.sns.util.EmojiFilter;
...
public class WXEntryActivity extends Activity implements IWXAPIEventHandler{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(isLogin){
			//TODO 加载微信登陆的界面
			Logger.v(TAG, "onCreate", " weixin login  -------->");
			setContentView(R.layout.activity_weixin_page_login);
			iniHandler();
		}else{
			Logger.v(TAG, "onCreate", " weixin share  -------->");
	...
	public void onResp(BaseResp resp) {
		...
		if(resp.getType()==ConstantsAPI.COMMAND_SENDMESSAGE_TO_WX){
			//TODO 分享到微信返回
			Logger.v(TAG, "onResp", " weixin share !  resp errorCode : " + resp.errCode + " openId : " + resp.openId);
			...
			//TODO 请求授权 返回
			Logger.v(TAG, "onResp", " weixin login !  resp errorCode : "+resp.errCode + " openId : " + resp.openId);
	...
	private void getUserInfo(final SendAuth.Resp resp){
		Logger.v(TAG, "getUserInfo", " getUserInfo start !   resp.state : " + resp.state + " resp.code : " + resp.code);
	...
	public void onReq(BaseReq req) {
		Logger.v(TAG, "onReq", " req.getType() : "+req.getType());
		switch (req.getType()) {
