...
//	private static final boolean DEBUG = Logger.DEBUG && false;
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (view instanceof MyImageView) {
			...
			float min_distance = PhotoDetailsActivity.screenWidth / 4f;
//			Logger.d(DEBUG,"xdistance=" + xdistance + ",min_distance="
//					+ min_distance);
//			System.out.println("e1 = "+e1.getX()+"   e2 = "+e2.getX()+"   xdistance : "+xdistance);
			...
			if ((int) width <= PhotoDetailsActivity.screenWidth
					&& (int) height <= PhotoDetailsActivity.screenHeight)// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				...
				if (distanceX > 0)// 向左滑动
				{
					if (r.left > 0 || right < PhotoDetailsActivity.screenWidth) {// 判断当前ImageView是否显示完全
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
				...
				{
					if (r.right < PhotoDetailsActivity.screenWidth || left > 0) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
						imageView.postTranslate(-distanceX, -distanceY);
					}
	...
	private boolean isScrollingLeft(float e1, float e2) {
		return e2 > e1;
	}
	...
	private float calXdistance(float e1, float e2) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof MyImageView) {
				...
				x2 = event.getX();
				float xdistance = calXdistance(x1, x2);
				float min_distance = PhotoDetailsActivity.screenWidth / 4f;
//				System.out.println("x1 = "+x1+"  x2 = "+x2+"  distance : "+xdistance);
				imageView = (MyImageView) view;
				float width = imageView.getScale() * imageView.getImageWidth();
				...
				if((int) width <= PhotoDetailsActivity.screenWidth
						&& (int) height <= PhotoDetailsActivity.screenHeight
						&& kEvent != KEY_INVALID
						&& isScroll) { // 是否切换上一页或下一页
					onKeyDown(kEvent, null); 
//					System.out.println("kEvent = "+kEvent);
					pageSelectedListener.pageSeclected(kEvent);
					kEvent = KEY_INVALID;
				}
				// Logger.LOG("onTouchEvent", "width=" + width + ",height="
				// + height + ",screenWidth="
				// + PhotoDetailsActivity.screenWidth + ",screenHeight="
				// + PhotoDetailsActivity.screenHeight);
				if ((int) width <= PhotoDetailsActivity.screenWidth
						&& (int) height <= PhotoDetailsActivity.screenHeight)// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				if (top < 0 && bottom < PhotoDetailsActivity.screenHeight) {
//					imageView.postTranslateDur(-top, 200f);
					imageView.postTranslateDur(PhotoDetailsActivity.screenHeight
							- bottom, 200f);
				}
				if (top > 0 && bottom > PhotoDetailsActivity.screenHeight) {
//					imageView.postTranslateDur(PhotoDetailsActivity.screenHeight
//							- bottom, 200f);
				...
				if(left<0 && right< PhotoDetailsActivity.screenWidth){
//					imageView.postTranslateXDur(-left, 200f);
					imageView.postTranslateXDur(PhotoDetailsActivity.screenWidth
							- right, 200f);
				}
				if(left>0 && right>PhotoDetailsActivity.screenWidth){
//					imageView.postTranslateXDur(PhotoDetailsActivity.screenWidth
//							- right, 200f);
	...
	public interface onPageSelectedListener{
		abstract void pageSeclected(int kEvent);
	}
