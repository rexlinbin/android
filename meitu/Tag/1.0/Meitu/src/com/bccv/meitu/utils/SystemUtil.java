...
import com.bccv.meitu.model.InstallAppInfo;
...
public class SystemUtil {
	...
	/**
	 * 获取{@link com.fone.player.util.SystemUtil}对象<br>
	 * 单例方法<br>
	 * 同步
	 * 
	 * @return {@link com.fone.player.util.SystemUtil}对象
	 */
	public static SystemUtil getInstance() {
		
		if(sContext==null){
			throw new IllegalArgumentException(" init(Context context) should be run first !!! ");
		}
	...
	/**
	 * 初始化{@link com.fone.player.util.SystemUtil}
	 * 
	 * @param context
	 *            当前上下文
	 */
	 ...
	 * @return
	 */
	public static String getFoneCacheFolder(int type) {
		String status = Environment.getExternalStorageState();
		Logger.v(TAG, "getFoneCacheFolder status:", status);
		if (!status.equals(Environment.MEDIA_MOUNTED)) {
			String path = null;
			switch (type) {
			case DOWNLOAD_APK_TYPE:
				path = SystemUtil.getInstance().getRootPath()
		...
		Logger.v(TAG, "getFoneCacheFolder childFile:", "childFile getPath : "
	...
	public void saveToPathMap(ArrayList<String> arrayPath) {
		Logger.v(TAG, "saveToPathMap", "arrayPath=" + arrayPath);
		mPathMap.clear();
		String sysInternalPath = Environment.getExternalStorageDirectory()
				.getPath();
		Logger.v(TAG, "saveToPathMap", "sysInternalPath=" + sysInternalPath);
		...
		if (android.os.Build.MODEL.equals("HS-U939")) {
			for (int i = 0; i < arrayPath.size(); i++) {
				...
				if (path.equals(sysInternalPath) && new File(path).exists()) {// 澶栫疆sd
					Logger.v(TAG, "saveToPathMap", "HS-U939 EXTERNAL=" + path);
					...
					if (!path.equals(sysInternalPath)
						...
						Logger.v(TAG, "saveToPathMap", "HS-U939 INTERNAL=" + path);
		...
		if (isSpecialPhone(android.os.Build.MODEL)) {
			Logger.v(TAG, "saveToPathMap", "isSpecialPhone=true");
			for (int i = 0; i < arrayPath.size(); i++) {
				String path = arrayPath.get(i);
				if (path.equals(sysInternalPath)) { // 外置sd
					if (new File(path).exists() && new File(path).canWrite()) {
						Logger.v(TAG, "saveToPathMap", "EXTERNAL_PATH=" + path);
						mPathMap.put(EXTERNAL_PATH, path);
					} else {
						Logger.v(TAG, "saveToPathMap", "not write INTERNAL_PATH="
								+ path);
					}
				} else {// 外置sd
					if (new File(path).exists() && new File(path).canWrite()
							&& filterStoragePath(path)) {
						Logger.v(TAG, "saveToPathMap", "INTERNAL_PATH=" + path);
						mPathMap.put(INTERNAL_PATH, path);
					} else {
						Logger.v(TAG, "saveToPathMap", "not write INTERNAL_PATH="
								+ path);
			...
			Logger.v(TAG, "saveToPathMap", "isSpecialPhone=false");
			for (int i = 0; i < arrayPath.size(); i++) {
				String path = arrayPath.get(i);
				if (path.equals(sysInternalPath)) { // 内置sd
					...
					if (new File(path).exists() && new File(path).canWrite()
							&& filterStoragePath(path)) {
						Logger.v(TAG, "saveToPathMap", "EXTERNAL=" + path);
						mPathMap.put(EXTERNAL_PATH, path);
					} else {
						Logger.v(TAG, "saveToPathMap", "not write EXTERNAL_PATH="
								+ path);
					}
				}
			}
	...
	public boolean isSpecialPhone(String phone) {
		Logger.v(TAG, "isSpecialPhone", "phone : " + phone);
		String phones[] = sContext.getResources().getStringArray(
				R.array.cache_phone_facturers);
		Logger.v(TAG, "isSpecialPhone", "phones : " + phones);
		boolean res = false;
	...
	private String createPath(String path) {
		path += File.separator + sContext.getResources().getString(R.string.cache_app_name) 
	 ...
	 * @return
	 */
	public PackageInfo checkApkExist(Context context, String packageName) {
	...
	public void installApp(String filePath) {
		...
		if (file.exists()) {
			...
			Logger.e(TAG, "installApp", " file is not exists !!! ");
		}
	...
	public static void openApp(String packageName,Context context) { 
			...
			Logger.v(TAG, "openApp", "packageName : " + packageName);
	...
	private void showUninstallAPKIcon(String apkPath) {  
        ...
        try {  
            // apk鍖呯殑鏂囦欢璺緞  
            // 杩欐槸涓�涓狿ackage 瑙ｉ噴鍣�, 鏄殣钘忕殑  
            // 鏋勯�犲嚱鏁扮殑鍙傛暟鍙湁涓�涓�, apk鏂囦欢鐨勮矾寰�  
            ...
            // 杩欎釜鏄笌鏄剧ず鏈夊叧鐨�, 閲岄潰娑夊強鍒颁竴浜涘儚绱犳樉绀虹瓑绛�, 鎴戜滑浣跨敤榛樿鐨勬儏鍐�  
            ...
            // 搴旂敤绋嬪簭淇℃伅鍖�, 杩欎釜鍏紑鐨�, 涓嶈繃鏈変簺鍑芥暟, 鍙橀噺娌″叕寮�  
            // ApplicationInfo info = mPkgInfo.applicationInfo;  
            Field appInfoFld = pkgParserPkg.getClass().getDeclaredField("applicationInfo");  
            ApplicationInfo info = (ApplicationInfo) appInfoFld.get(pkgParserPkg);  
            // uid 杈撳嚭涓�"-1"锛屽師鍥犳槸鏈畨瑁咃紝绯荤粺鏈垎閰嶅叾Uid銆�  
...
//            }  
	 ...
	 public static boolean hasRooted(){
	  ...
	  try {
	   ...
	   // TODO Auto-generated catch block
		  Logger.e(TAG,"hasRooted", "Unexpected error - Here is what I know: "+e.getMessage());
	   return false;
	  }	catch (InterruptedException e) {
		// TODO: handle exception
		   Logger.e(TAG,"hasRooted", "InterruptedException error : "+e.getMessage());
		   return false;
	  } finally {    
	   try {    
	    if (out != null) 
	...
	public static ArrayList<InstallAppInfo> getInstallAppInfos(Context context){
		 ...
		 ArrayList<InstallAppInfo> InstallAppInfoList = new ArrayList<InstallAppInfo>(installedPackages.size());
		 ...
		 for (PackageInfo installedPackage : installedPackages) {
			 if((installedPackage.applicationInfo.flags&ApplicationInfo.FLAG_SYSTEM)==0){
				 ...
				 installAppInfo.setVersion(String.valueOf(installedPackage.versionCode));
	...
	public static boolean isNetOkWithToast(Context context) {
		...
		if (getCurrentNetType(context) == 0) {
			Logger.v(TAG, "isNetOkWithToast", "no_network_toast");
			Toast.makeText(context, "当前无网络，请检查网络设置",
	...
	public static int getCurrentNetType(Context context) {
		...
		if (null != activeNetInfo) {
			Logger.v(TAG, "getNetType : ", activeNetInfo.toString());
