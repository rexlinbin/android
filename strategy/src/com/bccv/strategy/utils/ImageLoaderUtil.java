...
//	private static DisplayImageOptions roundedImageOptions ;
	...
	private static DisplayImageOptions strategyshowImageOptions ;
...
//	}
	...
	public static DisplayImageOptions getStrategyshowImageOptions(){
		
		if(strategyshowImageOptions==null){
			strategyshowImageOptions = new DisplayImageOptions.Builder()
			.showImageOnLoading(R.drawable.default_160) // 设置图片在下载期间显示的图片
			.showImageForEmptyUri(R.drawable.default_160)// 设置图片Uri为空或是错误的时候显示的图片
			.showImageOnFail(R.drawable.default_160) // 设置图片加载/解码过程中错误时候显示的图片
			.cacheInMemory(true)// 设置下载的图片是否缓存在内存中
			.cacheOnDisk(true)// 设置下载的图片是否缓存在SD卡中
			.considerExifParams(true) // 是否考虑JPEG图像EXIF参数（旋转，翻转）
			.imageScaleType(ImageScaleType.EXACTLY)// 设置图片以如何的编码方式显示
		...
		return strategyshowImageOptions;
