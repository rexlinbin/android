...
import com.bccv.strategy.ui.adapter.MyQuestionAdapter;
import com.bccv.strategy.ui.adapter.PersonalZoneAdapter;
...
public class PersonalZoneActivity extends BaseActivity implements 
	...
	private void initHeads() {
		...
		if (!isSelf) {
			...
			two_hot_tv.setText(getResources().getString(R.string.other_collection));
			two_fresh_tv.setText(getResources().getString(R.string.other_question));
			getUserInfo(user_id);
		}else {
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (cur == LEFT && sharedApps != null && position > 2 &&
			...
			Intent intent = new Intent(PersonalZoneActivity.this,StrategyDetailsActivity.class);
			intent.putExtra(BackGroundView.BACKGROUND_COLOR, backgrounds);
			intent.putExtra(StrategyDetailsActivity.RAID_ID, sharedApps.get(position-3).getId());
			intent.putExtra(StrategyDetailsActivity.GAME_ID, sharedApps.get(position-3).getGame_id());
			startActivity(intent);
...
//				isRightLoadMore = false;
	...
	private void getSharedAppSet(String user_id,int p,int num) {
		if (SystemUtil.isNetOkWithToast(mContext)) {
			showDialog(true);
			NetWorkAPI.user_focus_r(mContext, p, new HttpCallback() {
