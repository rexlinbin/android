...
import com.bccv.strategy.model.ReplyBean;
...
public class Answer2PersonActivity extends BaseActivity implements
	...
	private void requestData(int p) {
		...
		if (SystemUtil.isNetOkWithToast(this.getApplicationContext())) {
			...
			NetWorkAPI.find_answer(mContext, p, question.getId(),
					new HttpCallback() {
	...
	private void doComment() {
		...
		if (SystemUtil.isNetOkWithToast(this.getApplicationContext())) {
			...
			if(UserInfoManager.isLogin()){
				...
				NetWorkAPI.add_answer(this.getApplicationContext(), answer,
	...
	private void reportZan() {
		...
		if (hasZan && stringBuilder.length() >= 2) {
			...
			NetWorkAPI.digg_a(mContext, stringBuilder.toString(),
					new HttpCallback() {
