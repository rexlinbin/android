...
import com.bccv.strategy.sns.sina.SinaShare;
import com.bccv.strategy.sns.tencent.qq.QQshare;
...
public class SNSShareManager {
	...
	public void shareMessage(Activity activity, ShareInfo shareInfo) {
		...
		if(mShareStateListener!=null){
			mShareStateListener.shareAction(ShareStateListener.SHARING, "sharing",shareInfo.SNSType);
		}
		if(shareInfo==null||TextUtils.isEmpty(shareInfo.text)){
			if(mShareStateListener!=null){
				mShareStateListener.shareAction(ShareStateListener.SHARE_FAILED, "分享失败",shareInfo.SNSType);
			}
			return;
		}else if(shareInfo.text.length()>65){
			shareInfo.text = shareInfo.text.substring(0, 65)+"...";
		}
...
//		}
	...
	private void share2QQ(Activity activity, ShareInfo shareInfo){
		...
		L.v(TAG, "share2QQ", shareInfo.toString());
...
//		TencentWeiboShare.getInstance(mContext).share2tencentWB(activity, shareInfo.text, shareInfo.imageUrl, bitmap, mShareStateListener);
	 ...
	 *
	 */
	public static interface ShareStateListener{
