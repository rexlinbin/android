...
public class ApiClient {
	...
	private static HttpClient getHttpClient() {
		...
		Context context = ApplicationManager.getGlobalContext();
		WifiManager wifiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
		if (!wifiManager.isWifiEnabled()) {
			String str = ApplicationManager.getGlobalContext().getString(R.string.tel_str);
	...
	public static InputStream http_get(String url, Map<String, Object> params) { 
		...
		do {
			try {
				...
				time++;
				if (time < RETRY_TIME) {
					L.v(TAG, "connection failed !  reconnection ...  times : " + time);
					try {
						Thread.sleep(1000);
	...
	public static InputStream http_post(String url, Map<String, Object> params,Map<String,File> files){
		...
		do{
			try{
				...
				L.e(TAG, "http_post",e.getMessage());
				return null;
			} finally {
				if(null!=httpClient){
					httpClient.getConnectionManager().closeExpiredConnections();
