	...
	private void showProgressNotifi(Download download){
		if(download.showProgress()){
			...
			if (runningNotification != null
				...
				// 更新进度
				runningNotification.contentView.setTextViewText(R.id.download_title,
						download.getDownloadFileName());
				runningNotification.contentView.setTextViewText(R.id.download_progress_text,
						"已下载" + download.getDownloadProgress() + "%");
				runningNotification.contentView.setProgressBar(R.id.download_progress,
						100, (int)download.getDownloadProgress(), false);
				
				Intent buttonIntent = new Intent(DownloadService.DOWNLOAD_BTN_ACTION);
				buttonIntent.putExtra(DownloadService.DOWNLOAD_INFO, download.getDownloadUrl());
				buttonIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				PendingIntent pendButtonIntent = PendingIntent.getBroadcast(mContext,
						download.getProgressTag(), buttonIntent, PendingIntent.FLAG_UPDATE_CURRENT);
				runningNotification.contentView.setOnClickPendingIntent(R.id.download_cancel_btn, pendButtonIntent);
				
//				buttonIntent.putExtra(NotificationReceiver.NOTIFICATION_TEMP, tmpID);
	...
	public void stopDownload(String url){
		DownloadRunnable downloadRunnable = mDownloadRunnableMap.get(url);
		if(downloadRunnable!=null){
			downloadRunnable.setIsStop(true);
		}
		removeProgressNotifi(downloadRunnable.getDownload());
	}
