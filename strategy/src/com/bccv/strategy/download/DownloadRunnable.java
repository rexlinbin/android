package com.bccv.strategy.download;
...
public class DownloadRunnable implements Runnable {
	 ...
	 * 连接超时60秒
	 */
	private static final int mConnectTimeout = 10 * 1000;
	/**
	 * 读取超时30秒
	 */
	private static final int mReadTimeout = 10 * 1000;
	/**
	 * 重试次数
	 */
	...
	public void run() {
		...
		if(mDownload.isApk()){
			filePath.append(SystemUtil
					.getCacheFolder(SystemUtil.DOWNLOAD_APK_TYPE));
			filePath.append(File.separator);
			filePath.append(mDownload.getDownloadFileName());
			filePath.append(".apk");
		}else{
			filePath.append(SystemUtil
					.getCacheFolder(SystemUtil.DOWNLOAD_IMAGE_TYPE));
			filePath.append(File.separator);
			filePath.append(mDownload.getDownloadFileName());
		}
		mDownload.setDownloadStoragePath(filePath.toString());
		
		L.v(TAG, "run", "filePath : " + filePath.toString());
	...
	private void limitSpeedDownloadFile(long speed){
		...
		if(mDownload.showProgress()){
			mDownload.setDownloadProgress(0);
			Message msgStart = mHandler.obtainMessage();
			msgStart.obj = mDownload;
			msgStart.what = DownloadManager.SHOW_PROGRESS;
		...
		try{
			...
			if (!file.exists()) { // TODO 如果文件不存在
				...
				if(file.length()==mDownload.getDownloadTotalSize()){
					L.v(TAG, "limitSpeedDownloadFile", "file.length()==mDownload.getDownloadTotalSize() : true");
					mDownload.setDownloadAlreadySize(file.length());
					
					if(mDownload.showProgress()){
						mDownload.setDownloadProgress(100);
						Message msgStart = mHandler.obtainMessage();
						msgStart.obj = mDownload;
						msgStart.what = DownloadManager.SHOW_PROGRESS;
						msgStart.sendToTarget();	
					...
					L.v(TAG, "limitSpeedDownloadFile", "file.length()==mDownload.getDownloadTotalSize() : false");
					file.delete();
					file.createNewFile();
					mDownload.setDownloadAlreadySize(0);
					mDownload.setDownloadTotalSize(length);
			...
			if (speed != 0) {
				...
				L.v(TAG, "limitSpeedDownloadFile", "speed=" + speed + " sleep="
						+ sleep + " url=" + mDownload.getDownloadUrl());
			}
			int maxCount = (int) Math.floor((length - start) / pack) + 1;
			L.v(TAG, "limitSpeedDownloadFile", "maxCount=" + maxCount + " url="
...
//			int oldPrecent = 0;
			...
			while ((len = inputStream.read(buf)) != -1) {
				...
				if (file.canWrite()) {
					...
					L.e(TAG, "limitSpeedDownloadFile", "DOWNLOAD_IO_EXCEPTION"
			...
			if(mIsStop){
				L.v(TAG, "run", " DOWNLOAD_CANCEL !!! ");
				mDownload.setDownloadAlreadySize(mDownload
						.getDownloadTotalSize());
				mDownload.setDownloadTotalSize(length);
				mDownload.setErrorCode(Download.DOWNLOAD_CANCEL);
			}else{
				mDownload.setDownloadAlreadySize(mDownload
						.getDownloadTotalSize());
				mDownload.setDownloadTotalSize(length);
			...
			if (mCurrentRetryNum <= RETRY_MAX_NUM) {
				L.w(TAG, "limitSpeedDownloadFile",
				...
				//TODO 连接失败 下载失败
				L.e(TAG, "limitSpeedDownloadFile", "SocketException "
			...
			//TODO 无效的url 下载失败
			L.e(TAG, "limitSpeedDownloadFile", "MalformedURLException "
			...
			if (e instanceof FileNotFoundException) {
				L.e(TAG, "limitSpeedDownloadFile", "FileNotFoundException "
						+ mDownload.getDownloadFileName() + "下载失败,连接地址失效");
			} else {
				L.e(TAG, "limitSpeedDownloadFile", "DOWNLOAD_IO_EXCEPTION e="
			...
			}else if(mDownload.getErrorCode()==Download.DOWNLOAD_CANCEL){
