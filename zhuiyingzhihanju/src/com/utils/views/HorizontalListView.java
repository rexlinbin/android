...
public class HorizontalListView extends AdapterView<ListAdapter> {
	...
	public void setSelection(int position) {
        ...
        int width_phone = wm.getDefaultDisplay().getWidth(); // 获取到屏幕宽度（像素） 
        int screenNum = 0;
		int positionX = 0;
		int maxWidth = 0;
		...
		for (int i = 0; i < this.getAdapter().getCount(); i++) {
			if (i < position) {
				positionX = positionX + this.getChildAt(0).getWidth();
			}
			maxWidth += this.getChildAt(0).getWidth();
			if (screenNum == 0) {
				if (maxWidth >= width_phone) {
					screenNum = i + 1;
				}
			}
		}
		
		if (position < screenNum) {
			positionX = 0;
		}
