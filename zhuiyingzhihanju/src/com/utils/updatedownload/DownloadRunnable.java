...
public class DownloadRunnable implements Runnable {
	...
	public void run() {
		...
		if(mDownload.isApk()){
			filePath.append(SystemUtil
					.getMeiTuCacheFolder(SystemUtil.DOWNLOAD_APK_TYPE));
		}else{
			filePath.append(SystemUtil
					.getMeiTuCacheFolder(SystemUtil.DOWNLOAD_IMAGE_TYPE));
		}
		...
		Logger.v("run", "filePath : " + filePath.toString());
	...
	private void limitSpeedDownloadFile(long speed){
		Logger.v("limitSpeedDownloadFile",
		...
		try{
			...
			if (!file.exists()) { // TODO 如果文件不存在
				...
				Logger.v("limitSpeedDownloadFile", "create new file path="
				...
				if(file.length()==mDownload.getDownloadTotalSize()){
					Logger.v("limitSpeedDownloadFile", "file.length()==mDownload.getDownloadTotalSize() : true");
					mDownload.setDownloadAlreadySize(file.length());
				}else{
					Logger.v("limitSpeedDownloadFile", "file.length()==mDownload.getDownloadTotalSize() : false");
					file.delete();
					file.createNewFile();
					mDownload.setDownloadAlreadySize(0);
					mDownload.setDownloadTotalSize(length);
			...
			if (speed != 0) {
				...
				Logger.v("limitSpeedDownloadFile", "speed=" + speed + " sleep="
						+ sleep + " url=" + mDownload.getDownloadUrl());
			}
			int maxCount = (int) Math.floor((length - start) / pack) + 1;
			Logger.v("limitSpeedDownloadFile", "maxCount=" + maxCount + " url="
...
//			int oldPrecent = 0;
			...
			while ((len = inputStream.read(buf)) != -1) {
				...
				if (file.canWrite()) {
					...
					Logger.e("limitSpeedDownloadFile", "DOWNLOAD_IO_EXCEPTION"
			...
			if (mCurrentRetryNum <= RETRY_MAX_NUM) {
				Logger.w("limitSpeedDownloadFile",
				...
				//TODO 连接失败 下载失败
				Logger.e("limitSpeedDownloadFile", "SocketException "
			...
			//TODO 无效的url 下载失败
			Logger.e("limitSpeedDownloadFile", "MalformedURLException "
			...
			if (e instanceof FileNotFoundException) {
				Logger.e("limitSpeedDownloadFile", "FileNotFoundException "
						+ mDownload.getDownloadFileName() + "下载失败,连接地址失效");
			} else {
				Logger.e("limitSpeedDownloadFile", "DOWNLOAD_IO_EXCEPTION e="
