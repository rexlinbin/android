...
public class M3U8Utils {
	...
	public M3U8Utils(Context context, String model_id, String modelType_id, String modelImage_url, String model_title,
		...
		downloadRootPath = GlobalParams.downloadPath + "/" + model_id + "a"+ modelType_id + "/" + source + "/";
		downloadManager = DownloadUtil.getDownloadManager();

	}

	public void initDownload(String downloadUrl, int episode_id, boolean isM3U8, String useragent) {
//		downloadUrl = "https://secure-appldnld.apple.com/itunes12/031-69285-20160802-7E7A134A-552B-11E6-B37E-686CECD541CE/iTunes6464Setup.exe";
		downloadManager.setUserAgent(useragent);
		this.isM3U8 = isM3U8;
		if (isEpisode) {
	...
	private void startDownload(int episode_id) {
		...
		try {
			for (int i = 0; i < downloadList.size(); i++) {// downloadList.size()
				...
				if (index > 0) {
					String fileName = model_id + ".mp4";
					downloadManager.addNewestDownload(url, fileName + "", downloadPath + fileName, true, true, null,
							model_id, modelType_id, model_title, modelImage_url, isEpisode, episode_id, localUrl,
							source, source_id, hd, isM3U8);
//					downloadManager.addNewMp4Download(url, fileName + "", downloadPath + fileName, true, true, null,
//							model_id, modelType_id, model_title, modelImage_url, isEpisode, episode_id, localUrl,
//							source, source_id, hd, isM3U8);
				}
			}
	...
	private void startDownloadM3U8(int episode_id) {
		...
		try {
			for (int i = 0; i < downloadList.size(); i++) {// downloadList.size()
				...
				downloadManager.addNewestDownload(downloadList.get(i), i + "", downloadPath + i + ".ts", true, true,
