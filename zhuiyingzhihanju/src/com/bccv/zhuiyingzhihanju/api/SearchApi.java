...
public class SearchApi extends AppApi {
	...
	public List<SearchRecom> getRecomSearchList(
			String count) {
		HttpClientUtil util = new HttpClientUtil();
		Map<String, String> params = new HashMap<String, String>();
		String result = util.sendGet(Url.SearchRecom_list, params);
		if (result != null) {
			Log.e("getRecomSearchList", result);
		} else {
			Log.e("getRecomSearchList", "null");
		}
		if (!StringUtils.isEmpty(result)) {
			try {
				JSONObject jsonObject = new JSONObject(result);
				if (checkResponse(jsonObject)) {
					...
					if (!StringUtils.isEmpty(rtnStr) && !rtnStr.equals("null")) {
						...
						List<SearchRecom> list = null;
						list = JSON.parseArray(rtnStr, SearchRecom.class);
