...
import com.utils.tools.MyApplication;
...
public class SystemUtil {
	...
	private static boolean checkFsWritable() {
		...
		// Logger.d("checkFsWritable directoryName ==   "
	...
	private static File getExternalCacheDir(Context context) {
		...
		if (!appCacheDir.exists()) {
			if (!appCacheDir.mkdirs()) {
				Logger.w("SystemUtil", "Unable to create external cache directory");
				return null;
			}
			try {
	...
	public static String getMeiTuCacheFolder(int type) {
		String status = Environment.getExternalStorageState();
		Logger.v("getFoneCacheFolder status:", status);
		if (!status.equals(Environment.MEDIA_MOUNTED)) {
			String path = null;
			switch (type) {
		...
		Logger.v("getFoneCacheFolder childFile:", "childFile getPath : "
	...
	public void saveToPathMap(ArrayList<String> arrayPath) {
		Logger.v("saveToPathMap", "arrayPath=" + arrayPath);
		mPathMap.clear();
		String sysInternalPath = Environment.getExternalStorageDirectory()
				.getPath();
		Logger.v("saveToPathMap", "sysInternalPath=" + sysInternalPath);
		...
		if (android.os.Build.MODEL.equals("HS-U939")) {
			for (int i = 0; i < arrayPath.size(); i++) {
				...
				if (path.equals(sysInternalPath) && new File(path).exists()) {// 澶栫疆sd
					Logger.v("saveToPathMap", "HS-U939 EXTERNAL=" + path);
					...
					if (!path.equals(sysInternalPath)
						...
						Logger.v("saveToPathMap", "HS-U939 INTERNAL="
								+ path);
		...
		if (isSpecialPhone(android.os.Build.MODEL)) {
			Logger.v("saveToPathMap", "isSpecialPhone=true");
			for (int i = 0; i < arrayPath.size(); i++) {
				String path = arrayPath.get(i);
				if (path.equals(sysInternalPath)) { // 外置sd
					if (new File(path).exists() && new File(path).canWrite()) {
						Logger.v("saveToPathMap", "EXTERNAL_PATH=" + path);
						mPathMap.put(EXTERNAL_PATH, path);
					} else {
						Logger.v("saveToPathMap",
								"not write INTERNAL_PATH=" + path);
					}
				} else {// 外置sd
					if (new File(path).exists() && new File(path).canWrite()
							&& filterStoragePath(path)) {
						Logger.v("saveToPathMap", "INTERNAL_PATH=" + path);
						mPathMap.put(INTERNAL_PATH, path);
					} else {
						Logger.v("saveToPathMap",
								"not write INTERNAL_PATH=" + path);
			...
			Logger.v("saveToPathMap", "isSpecialPhone=false");
			for (int i = 0; i < arrayPath.size(); i++) {
				String path = arrayPath.get(i);
				if (path.equals(sysInternalPath)) { // 内置sd
					...
					if (new File(path).exists() && new File(path).canWrite()
							&& filterStoragePath(path)) {
						Logger.v("saveToPathMap", "EXTERNAL=" + path);
						mPathMap.put(EXTERNAL_PATH, path);
					} else {
						Logger.v("saveToPathMap",
	...
	public String getRootPath() {
		...
		boolean isMounted =  AppConfig.getisSD();
//		boolean isMounted = isSDCardMounted();
	...
	public static void openApp(String packageName, Context context) {
		...
		Logger.v("openApp", "packageName : " + packageName);
	...
	public static boolean isNetOkWithToast(Context context) {
		...
		if (getCurrentNetType(context) == 0) {
			Logger.v("isNetOkWithToast", "no_network_toast");
	...
	public static int getCurrentNetType(Context context) {
		...
		if (null != activeNetInfo) {
			Logger.v("getNetType : ", activeNetInfo.toString());
	...
	public static String getChannelCode() {
		String code = "own";
		try {
			ApplicationInfo info=MyApplication.getApplicationInstance().getPackageManager()
					.getApplicationInfo(MyApplication.getApplicationInstance().getPackageName(),PackageManager.GET_META_DATA);
			code =info.metaData.getString("InstallChannel");
		} catch (Exception e) {
			Logger.e("getChannelCode", e.getClass().getSimpleName());
			code = "own";
		}
		return code;
	}
	...
	public static String getPhoneUdid(){
		...
		udidStr = ((TelephonyManager) MyApplication.getApplicationInstance().getSystemService(Context.TELEPHONY_SERVICE))
				.getDeviceId();
		return udidStr;
	}
