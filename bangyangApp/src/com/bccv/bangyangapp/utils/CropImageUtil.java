...
import com.bccv.bangyangapp.utils.clipping.CropImage;
...
public class CropImageUtil {
	...
	public void startPhotoZoom(Uri uri) {
		// 裁剪图片意图
		Intent intent = new Intent("bccv.bangyang.intent.action.CROP");
	...
	public void handleMyActivityResult(int requestCode, int resultCode,
			Intent data) {
		L.v("测试", "调用了工具handleMyActivityResult,:resultCode" + resultCode
				+ " requestCode:" + requestCode + " data:" + data);
		if (resultCode != 0) {
			switch (requestCode) {
			case GALLERY_REQUEST_CODE:
				if (data != null) {
					L.v("测试","data.getdata:"+data.getData());
			...
			case RESULT_REQUEST_CODE:
				L.v("测试", "结果返回data:" + data + " cropImageListener:"
						+ cropImageListener);
