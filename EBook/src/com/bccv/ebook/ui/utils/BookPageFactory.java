package com.bccv.ebook.ui.utils;
...
public class BookPageFactory {
	...
	private static int m_fontSize = 40; // 字体大小
	private static int m_textColor = 0xff555555; // 字体颜色
	private static int m_backColor = 0xffcde5d5; // 背景颜色
	private static int m_lineSpacing = 4; // 行间距
	...
	private BookPageFactory(int w, int h) {
		mWidth = w;
		mHeight = h;
	 ...
	 * @return
	 */
	public static BookPageFactory build(int w, int h) {
		return new BookPageFactory(w, h);
	}
...
//		+ "/test.txt";
	...
	protected Vector<String> pageDown() {
		...
		while (lines.size() < mLineCount && m_mbBufEnd < m_mbBufLen) {
			...
			while (strParagraph.length() > 0&&nSize>0) {
				...
				if(l+m_fontSize<mVisibleWidth&&strParagraph.length()>nSize+1){
		...
		L.v(TAG, "pageDown", String.valueOf(System.currentTimeMillis()-startTime));
	...
	protected Vector<String> pageUp() {
		...
		L.v(TAG, "pageUp", String.valueOf(System.currentTimeMillis()-startTime));
