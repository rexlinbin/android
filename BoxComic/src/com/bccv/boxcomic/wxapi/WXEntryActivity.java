package com.bccv.boxcomic.wxapi;
...
import com.bccv.boxcomic.sns.util.EmojiFilter;
import com.bccv.boxcomic.tool.ApiClient;
...
//			//TODO 分享到微信返回
////			Logger.e(TAG+"onResp", " weixin share !  resp errorCode : " + resp.errCode + " openId : " + resp.openId);
//			
//			switch (resp.errCode) {
...
//		}else
        if (resp.getType() == ConstantsAPI.COMMAND_SENDAUTH && resp instanceof SendAuth.Resp){
			//TODO 请求授权 返回
			Logger.e(TAG+"onResp", " weixin login !  resp errorCode : "+resp.errCode + " openId : " + resp.openId);
	...
	private void getUserInfo(final SendAuth.Resp resp){
		Logger.e(TAG+"getUserInfo", " getUserInfo start !   resp.state : " + resp.state + " resp.code : " + resp.code);
