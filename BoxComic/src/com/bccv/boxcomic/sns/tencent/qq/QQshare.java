package com.bccv.boxcomic.sns.tencent.qq;
    	...
    	mTencent.login((Activity)mContext, "all",new IUiListener() {
    		...
    		public void onComplete(Object response) {
   			 ...
   			 try {
                    ...
                    Logger.v(TAG+"authQQ",  "fromShare : " + fromShare +" bundle : " + jsonObject.toString());
...
//			}
		...
		if(mShare==null){
			if(mShareStateListener!=null){
				mShareStateListener.shareAction(ShareStateListener.SHARE_FAILED, " mShare is null ");
			}
			return;
		}
		 ...
		 new Thread(new Runnable() {
			...
			public void run() {
				 mShare.shareToQzone(context, params,new IUiListener() {
					 @Override
					 public void onComplete(Object response) {
						 Logger.v(TAG+"ready2share onComplete", response + "");
						 if(mShareStateListener!=null){
							 mShareStateListener.shareAction(ShareStateListener.SHARE_SUCCESS, " Qzone share success ");
						 }
					 }
					 
					 @Override
					 public void onCancel() {
						 Logger.v(TAG+"ready2share onCancel", " Qzone share cancel ");
						 if(mShareStateListener!=null){
							 mShareStateListener.shareAction(ShareStateListener.SHARE_CANCEL, " Qzone share cancel ");
						 }
					 }
					...
					public void onError(UiError uiError) {
						...
						if(mShareStateListener!=null){
							mShareStateListener.shareAction(ShareStateListener.SHARE_FAILED, uiError.toString());
						}
					}
...
//		}
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Logger.v(TAG+"onActivityResult", "running");
		if(mTencent!=null){
			// Qzone分享 回调
