...
import com.bccv.zhuiying.adapter.DownloadEpisodeAdapter;
import com.bccv.zhuiying.adapter.DownloadSelectAdapter;
import com.bccv.zhuiying.adapter.SourceAdapter;
import com.bccv.zhuiying.api.MovieInfoApi;
import com.bccv.zhuiying.api.MovieUrlApi;
import com.bccv.zhuiying.fragment.EpisodeFragment;
import com.bccv.zhuiying.fragment.IntroFragment;
import com.bccv.zhuiying.fragment.TuijianFragment;
...
public class MovieInfoActivity extends BaseFragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		TCAgent.onPageStart(getApplicationContext(), "MovieInfoActivity");
	...
	private void initView() {
		...
		sourceListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				sourceTextView.setText(sourceList.get(position).getSource_name());
				if (isEpisode) {
					setEpisodes();
					resetEpisode();
				}
	...
	private void getData() {
			...
			public void handleResult(String result) {
				...
				if (movieInfo != null) {
					...
					if (sourceGetList != null && sourceGetList.size() > 0) {
						...
						sourceHeight = sourceCancel.getLayoutParams().height
								+ Function.setListViewHeightBasedOnChildren(sourceListView);
						sourceTextView.setText(sourceList.get(0).getSource_name());
		...
		new DataAsyncTask(callback, true) {
			...
			protected String doInBackground(String... params) {
				...
				if (isEpisode) {
					movieSourceList = movieUrlApi.getMovieEpidsodeUrlList(movie.getId(), movie.getType_id(), "1");
					if (movieSourceList != null && movieSourceList.size() > 0) {
						...
						for (int i = 0; i < movieSourceList.size(); i++) {
							...
							movieEpisode2.setDes(movieSourceList.get(i).getDes());
							movieEpisode2.setPeriods(movieSourceList.get(i).getPeriods());
							episodeGridList1.add(movieEpisode2);
						}
	...
	private void setEpisodes() {
		...
		for (int i = 0; i < movieSourceList.size(); i++) {
			// List<MovieUrl> movieUrls =
			// movieSourceList.get(i).getSource_text();
			...
			// for (int j = 0; j < episodeGridList.size(); j++) {
			// if
			// (episodeGridList.get(j).getId().equals(movieSourceList.get(i).getEpisodes_id()))
			// {
			// episodeGridList.remove(j);
			// break;
			// }
			// }
			//
		...
		episodesGridView.setNumColumns(gridViewNum);
		episodeDownAdapter.setGridViewNum(gridViewNum);
		episodeDownAdapter.notifyDataSetChanged();
	}
	...
	private void startDownload() {
		...
		m3u8Utils = new M3U8Utils(getApplicationContext(), movie.getId(), movie.getType_id(), movie.getImages(),
				movie.getTitle(), isEpisode, sourceList.get(currSource).getSource_name(),
				sourceList.get(currSource).getSource_url(), hd);
		if (isEpisode) {
			...
			for (int i = 0; i < episodeGridList.size(); i++) {
				...
				if (movieEpisode.isSelect()) {
					...
					if (list.contains(sourceList.get(currSource))) {
						for (int j = 0; j < list.size(); j++) {
							MovieUrl movieUrl = list.get(j);
							if (movieUrl.getSource_id().equals(sourceList.get(currSource).getSource_id())) {
								downloadNum++;
								getUrl(movieUrl.getSource_url(), Integer.parseInt(movieEpisode.getId()));
							}
						}
					} else {
						if (list.size() > 0) {
							MovieUrl movieUrl = list.get(0);
							downloadNum++;
							getUrl(movieUrl.getSource_url(), Integer.parseInt(movieEpisode.getId()));
						}
					}
	...
	public void startPlay(Movie playMovie, int position) {
		Intent intent = new Intent(getApplicationContext(), Video2DPlayerActivity.class);// AlivcVideo2DPlayerActivity
	...
	public void InitViewPager() {
		...
		if (isEpisode) {
			...
			episodeBundle.putSerializable("episode", (Serializable) episodeGridList1);
			...
			episodeBundle.putInt("gridViewNum", gridViewNum);
	...
	protected void onDestroy() {
		// TODO Auto-generated method stub
		super.onDestroy();
		TCAgent.onPageEnd(getApplicationContext(), "MovieInfoActivity");
	}
}
