...
import com.bccv.tianji.adapter.MyDownloadAdapter.ViewHolder;
import com.bccv.tianji.api.TcpApi;
import com.bccv.tianji.model.Game;
import com.nostra13.universalimageloader.core.ImageLoader;
import com.utils.tools.GlobalParams;
import com.utils.tools.MyUtils;
...
public class ClassifyListAdapter extends BaseAdapter {
	...
	public ClassifyListAdapter(Context context, List<Game> list){
	...
	public View getView(int arg0, View arg1, ViewGroup arg2) {
		...
		if (arg1 == null) {
			...
			viewHolder.iconImageView = (ImageView) arg1.findViewById(R.id.icon_imageView);
			viewHolder.titleTextView = (TextView) arg1.findViewById(R.id.title_textView);
			viewHolder.scoreTextView = (TextView) arg1.findViewById(R.id.score_textView);
			viewHolder.typeTextView = (TextView) arg1.findViewById(R.id.type_textView);
			viewHolder.introTextView = (TextView) arg1.findViewById(R.id.intro_textView);
			viewHolder.downloadTextView = (TextView) arg1.findViewById(R.id.download_textView);
			viewHolder.progressBar = (ProgressBar) arg1.findViewById(R.id.progressBar);
		...
		imageLoader.displayImage(game.getIcons(), viewHolder.iconImageView, GlobalParams.iconOptions);
		...
		viewHolder.introTextView.setText(game.getDown_num() + "次下载            " + game.getGame_size() + "GB");
		
		if (game.getUser_downloadstatus().equals("normal")) {
			viewHolder.downloadTextView.setText("下载");
			viewHolder.downloadTextView.setBackgroundResource(R.drawable.frame);
			viewHolder.progressBar.setVisibility(View.GONE);
		}else if (game.getUser_downloadstatus().equals("waiting")) {
			viewHolder.downloadTextView.setText("等待中");
			viewHolder.downloadTextView.setBackgroundResource(R.drawable.download_frame);
			viewHolder.progressBar.setVisibility(View.GONE);
		}else if (game.getUser_downloadstatus().equals("stop")) {
			viewHolder.downloadTextView.setText("继续");
			viewHolder.downloadTextView.setBackgroundResource(R.drawable.download_frame);
			viewHolder.progressBar.setVisibility(View.VISIBLE);
		}else if (game.getUser_downloadstatus().equals("download")) {
			viewHolder.downloadTextView.setText("已下载" + (int)game.getDownload_size() + "%");
			viewHolder.downloadTextView.setBackgroundResource(R.drawable.download_frame);
			viewHolder.progressBar.setVisibility(View.VISIBLE);
			viewHolder.progressBar.setProgress((int) game.getDownload_size());
		}else if (game.getUser_downloadstatus().equals("complete")) {
