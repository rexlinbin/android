...
import com.utils.views.SmoothImageView;
import com.utils.views.SmoothImageView.TransformListener;
...
public class ScreenShotActivity extends BaseActivity {
	...
	int mPosition = getIntent().getIntExtra("position", 0);
		...
		for (int i = 0; i < size; i++) {
			...
			if (!isDefault) {
				ImageLoader.getInstance().displayImage(mDatas.get(i), imageView, GlobalParams.Toppic);
			}else {
				imageView.setImageResource(R.drawable.top_bg);
			}
